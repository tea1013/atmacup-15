...
class Atmacup15Experiment(Experiment):
    def __init__(self, context: Atmacup15Context, config: Atmacup15ExperimentConfig, folds: List[int]) -> None:
    ...
    def train(self) -> TrainResult:
        ...
        for fold in self.folds:
            ...
            dataset = Atmacup15Dataset(train, valid, self.context.test)
    ...
    def save_oof(self, oof_prediction: ndarray, score: float) -> DataFrame:
        ...
        oof_df.to_csv(
            f"./oof/{self.config.exp_name}/v{self.config.version}_oof_cv{score:.4f}.csv",
            index=False,
        )
    ...
    def save_submission(self, test_prediction: ndarray, score: float) -> DataFrame:
        ...
        submission_df.to_csv(
            f"./submission/{self.config.exp_name}/v{self.config.version}_submission_cv{score:.4f}.csv",
            index=False,
        )
